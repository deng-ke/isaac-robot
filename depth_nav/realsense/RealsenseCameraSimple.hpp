...
// RealsenseCameraSimple is an Isaac codelet for the Realsense D435 camera that provides color and
// depth images. The sensor can also provide raw IR images, however this is currently not supported.
class RealsenseCameraSimple : public alice::Codelet {
 public:
  RealsenseCameraSimple();
  ~RealsenseCameraSimple();
  ...
  // The framerate of camera / depth image acquisition. Valid choices are: 60, 30, 15, 6. Note
  // that the depth camera supports higher framerates, however this is currently not supported as
  // color and depth are set to the same resolution.
  ISAAC_PARAM(int, framerate, 30);
  // If enabled the depth image is aligned to the color image to provide matching color and depth
 ...
 private:
  ...
  void configureDevice(const rs2::device& dev);
...
ISAAC_ALICE_REGISTER_CODELET(isaac::RealsenseCameraSimple)
